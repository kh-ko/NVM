...
import ValveParamPMItemModel 1.0
import ValveParamPMDlgModel 1.0
...
NWindow{ // khko_todo
    ...
    {
        var pageCount = Math.trunc((dlgModel.onCommandItemCount() - 1) / 10);

        console.debug("itemCount = " + dlgModel.onCommandItemCount());
        console.debug("pageCount = " + pageCount);

        for(var i = 0; i < pageCount + 1; i++)
        {
            pageBtnComponent.createObject(naviBtnBox, { "myPageNum": i });
        }
    }
    ...
    Item {
        ...
        Rectangle{
            ...
            NScrollView{
                ...
                delegate : Item{
                    property ValveParamPMItemModel paramItemModel : dlgModel.onCommandGetParamItem(idxValue)
...
/*
    ...
    Component{
        ...
        NButton{
            property int myPageNum : 0
            Layout.fillHeight: true; Layout.fillWidth: true; Layout.preferredWidth: 1
            enabled: dialog.progress === 100

            bgColor: dialog.currPageNum === myPageNum ? "#24A7FF" : "#FFFFFF"
            text.color: dialog.currPageNum === myPageNum ? "#FFFFFF" : "#000000"
            text.text: myPageNum + 1

            onClick: {
                if(dialog.currPageNum !== myPageNum)
                    dialog.movePage(myPageNum)
            }
        }
    }
}
