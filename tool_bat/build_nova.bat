@echo off
echo Building for nova configuration...

REM ..(NVM)/
echo.
echo Modifying company setting to COMPANY_NOVASEN...
if exist "..\source\service\localsetting\localsettingsprovider.h" (
    powershell -Command "(Get-Content '..\source\service\localsetting\localsettingsprovider.h') -replace '^(\s*int\s+mCompany\s*=\s*\(int\)ValveEnumDef::)[^;]+;', '    int     mCompany               = (int)ValveEnumDef::COMPANY_NOVA;' | Set-Content '..\source\service\localsetting\localsettingsprovider.h'"
    echo Company setting updated to COMPANY_NOVASEN successfully.
) else (
...
if exist "..\NVM_resource_nova_icon.rc" (
    copy "..\NVM_resource_nova_icon.rc" "..\NVM_resource.rc" 
    echo NVM_resource_nova_icon.rc copied to NVM_resource.rc successfully.
) else (
    echo Warning: NVM_resource_nova_icon.rc file not found!
)

REM Create build directory if it doesn't exist
if not exist "..\..\Binary\build_cursor\release_nova" (
    echo Creating release_nova directory...
    mkdir "..\..\Binary\build_cursor\release_nova"
)

cd ..\..\Binary\build_cursor\release_nova
if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to change directory to release_nova
...
if exist "..\..\..\Source\static_resource" (
    echo Copying static_resource files to nova folder...
