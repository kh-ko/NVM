...
public slots:
    ...
    Q_INVOKABLE void onCommandApply(int baudrateIdx, int parityIdx,int dataLenIdx, int stopBitIdx, int commandSetIdx, int secondAnswerIdx, int diOpenValveIdx, int diCloseValveIdx, int posRangeIdx, int pressureRange)
    {
        mIsWritten            = true           ;
...
private:
    enum eState{
        STATE_WRITE_CFG    =  0,
        STATE_WRITE_RANGE  = STATE_WRITE_CFG   + 1,
        STATE_READ_CFG     = STATE_WRITE_RANGE + 1,
        STATE_READ_RANGE   = STATE_READ_CFG    + 1,
